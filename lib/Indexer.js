      ...
      if (!validLicense) {
        ...
        if ((reverse?.indexableText || reverse?.['ldac:indexableText'] || reverse?.mainText || reverse?.['ldac:mainText'] ) && canIndexText) {
          ...
          function checkFile(roRef) {
            ...
            if (ro['ldac:indexableText']) return ro['ldac:indexableText'].some(f => f['@id'] === fileItem['@id']);
            if (ro.indexableText) return ro.indexableText.some(f => f['@id'] === fileItem['@id']);
            if (ro['ldac:mainText']) return ro['ldac:mainText'].some(f => f['@id'] === fileItem['@id']);
            if (ro.mainText) return ro.mainText.some(f => f['@id'] === fileItem['@id']);
          }
          if (!indexThisFile && reverse?.['ldac:indexableText']) indexThisFile = reverse['ldac:indexableText'].some(checkFile);
          if (!indexThisFile && reverse?.indexableText) indexThisFile = reverse.indexableText.some(checkFile);
          if (!indexThisFile && reverse?.['ldac:mainText']) indexThisFile = reverse['ldac:mainText'].some(checkFile);
