...
class Indexer {
  ...
  async indexCollection({item, crate, crateId, id, root, collectionMemberOf, collectionStack}) {
    ...
    if (!validLicense) {
      ...
      normalItem._origin = `${this.apiHost}/api/v1/oni/object/meta?id=${id}`;
  ...
  * */
  ...
  indexGeoLocation({item, normalItem}) {
    for (let c of item.contentLocation || []) {
      for (let g of c?.geo || []) {
        if (g?.asWKT) {
          normalItem._contentLocation = first(g.asWKT);
        }
      }
    }
    for (let s of item.spatialCoverage || []) {
      for (let g of s?.geo || []) {
        if (g?.asWKT) {
          normalItem._spatialCoverage = first(g.asWKT);
        }
      }
    }
  }
