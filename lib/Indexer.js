...
class Indexer {
  ...
  async getObjectbyId(id) {
    try {
      let url = `${this.protocol}://${this.host}/api/object?id=${encodeURIComponent(id)}`;
      this.log.info(url);
      const response = await fetch(url);
  ...
  async getTextFile({itemId, filePath}) {
    if (isEmpty(this.token)) {
      ...
      try {
        let url = `${this.protocol}://${this.host}/api/stream?id=${encodeURIComponent(itemId)}&path=${encodeURIComponent(filePath)}`;
        this.log.info(url);
        const response = await fetch(url, {
          method: 'post',
          headers: new Headers({
