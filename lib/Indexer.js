...
const {normalizeLdac, getContext} = require('./normalize');
...
class Indexer {
  ...
  constructor({configuration, repository, client}) {
    ...
    this.memberOfField = this.configuration.api.memberOfField || 'pcdm:memberOf';
    this.hasMemberField = this.configuration.api.hasMemberField || 'pcdm:hasMember';
  ...
  async findOcflObjects({memberOf, conformsTo, root, skip = [], collectionStack = []}) {
    ...
    for (const obj of ocflObjects) {
      ...
      if (skipRegExp?.test(obj.crateId)) {
        ...
        if(!context) {
          context = await getContext('https://w3id.org/ldac/context');
        }
        normalizeLdac(ocflCrate, context);
  ...
  async indexCollection({item, crate, crateId, id, root, collectionMemberOf, collectionStack, isOCFL}) {
    ...
    if (!validLicense) {
      ...
      if (!item[this.memberOfField]) {
        item._isTopLevel = 'true';
      } else {
        item[this.memberOfField] = undefined;
      }
    ...
    if (item[this.hasMemberField] && item[this.hasMemberField].length > 0) {
      for (const member of item[this.hasMemberField]) {
  ...
  async indexObject({item, crate, crateId, id, root, memberOf, collectionStack, isOCFL}) {
    ...
    if (!validLicense) {
      ...
      item.license = validLicense;
      //item._crateId = crateId; //We need this for the file downloads!
      if (item[this.memberOfField]) item[this.memberOfField] = undefined;
      const normalItem = crate.getTree({root: item, depth: 1, allowCycle: false});
  ...
  async indexFile({parent, crateId, item, hasPart, crate, _memberOf, root, collectionStack}) {
    ...
    if (fileItem) {
      ...
      if (!validLicense) {
        ...
        if ((reverse?.indexableText || reverse?.['ldac:indexableText'] || reverse?.mainText || reverse?.['ldac:mainText'] ) && canIndexText) {
          ...
          function checkFile(roRef) {
            ...
            const textProperties = ['ldac:indexableText', 'indexableText', 'ldac:mainText', 'mainText'];
            return textProperties.some(prop =>
              ro[prop]?.some(f => f['@id'] === fileItem['@id'])
            );
          }
          const reverseProperties = ['ldac:indexableText', 'indexableText', 'ldac:mainText', 'mainText'];
          for (const prop of reverseProperties) {
            if (!indexThisFile && reverse?.[prop]) {
              indexThisFile = reverse[prop].some(checkFile);
            }
            if (indexThisFile) break; // Stop checking once a match is found
          }
          if (indexThisFile) {
